...
RULES = [  # (pattern, why, files where it is the one right place)
    ...
    (re.compile(r"""(Text\(|errorText:|content:|title:|subtitle:)[^\n]*\$\{?(e|ex|err|error|exception)\b(?![.(])"""),
     "an exception's text on screen: use appErrorText", set()),
    (re.compile(r"\b(e|ex|err|error|exception)\.toString\(\)"), "an exception's text: use appErrorText", set()),
    (re.compile(r"suffixText:\s*'[A-Z]{3}'"), "a fixed currency code: use currencySymbol", set()),
    (re.compile(r"\$\{[^}]*/\s*100\b"), "money through a float: use amountText", set()),
    (re.compile(r"\$\{[^}]*\.hour\b"), "a time put together by hand: use formatTime",
     {"widgets/dates.dart", "features/pos/receipt_builder.dart"}),
