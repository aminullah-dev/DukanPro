...
| products update | product.manage (active), plus price.change when the price or currency changes, plus purchase.cost for a cost | `base_version` required; only `sku` (a correction; one another live product has is kept and flagged `sku_taken`), `name`, `sell_price_minor`, `sell_currency`, `is_active`, `track_stock`, `cost_minor` (a goods receipt's cost, in the selling currency); the currency one the shop's branches trade in (`PRICE_CURRENCY_INVALID`), as on REST |
...
| shifts update | sale.create for the shift's own seller, else sale.void (a manager) | `base_version` required; only an open shift closes (`SHIFT_ALREADY_CLOSED`); `status` closed and `counted_cash_minor`; the server sets the expected cash and the variance from its own rows |
