...
- **Barcode scans are layout-proof:** a keyboard-wedge scanner's keys are read from their physical position, so a Dari or Pashto keyboard layout cannot turn a code into Persian digits or Arabic letters. Codes typed with Persian digits are normalized before lookup.
  - A scan into a focused text field lands there too, typed by the layout: under Dari a code's letters come out as Persian letters. The till takes them back out and adds the product. It recognises them (`endsWithScan`, app/lib/widgets/digits.dart) by the same length, the same digits in either system, and a non-Latin letter wherever the code has a letter. The payment dialog checks every field, focused or not, so there a code of letters alone must match exactly and a Dari customer name is never taken for a scan.
